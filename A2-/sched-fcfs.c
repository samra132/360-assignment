 ...
 * Date: May 2026
 * File: sched.c
 * Description: FCFS CPU scheduling simulator (milestone version)
 */
...
int simulate(const job_t* input, int n, const sim_cfg_t* cfg, sim_metrics_t* out)
{
    ...
    for (int i = 0; i < tlen; i++) {
        if (timeline[i] >= 0 && prev >= 0 && timeline[i] != prev)
            ctx++;
